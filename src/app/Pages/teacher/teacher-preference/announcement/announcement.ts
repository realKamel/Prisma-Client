...
export class AnnouncementComponent implements OnInit {
  ...
  get charCount() { return this.annText.length; }
  get isActive()  { return !!this.activeAnn; }
  get btnLabel()  { return this.isActive ? 'تعديل الإعلان' : 'نشر الإعلان'; }

  onInput() { this.hasError = false; }

  publish() {
    const txt = this.annText.trim();
    if (!txt) { this.hasError = true; return; }
