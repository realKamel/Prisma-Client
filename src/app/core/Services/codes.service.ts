...
export class CodesService {
    ...
    getAcademicYears(): Observable<ApiResult<AcademicYear[]>> {
        return this.http.get<{ data: AcademicYear[]; succeeded: boolean }>(`${this.BASE}/academic-years`).pipe(
    ...
    getBatch(id: number): Observable<ApiResult<CodeBatch | null>> {
        return this.http.get<{ data: CodeBatch; succeeded: boolean }>(`${this.BASE}/batches/${id}`).pipe(
            map((res) => ({ data: res.data, fromFallback: false })),
            catchError(() => {
    ...
    createBatch(payload: {
        ...
        return this.http.post<{ data: { batchId: number; codes: string[] }; succeeded: boolean }>(
            `${this.BASE}/batches`, payload
        ).pipe(
